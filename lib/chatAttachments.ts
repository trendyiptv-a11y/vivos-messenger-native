...
const MAX_CAPTURE_VIDEO_SECONDS = 60
...
function capturedName(kind: "image" | "video", mimeType: string) {
  const fallbackExt = kind === "image" ? "jpg" : "mp4"
  const ext = extensionFromMime(mimeType, fallbackExt)
  return `${kind === "image" ? "photo" : "video"}-${Date.now()}.${ext}`
}
...
export async function captureChatPhoto(): Promise<PickedChatAttachment | null> {
  ...
  return {
    uri: asset.uri,
    name: safeFileName(asset.fileName || capturedName("image", mimeType)),
    mimeType,
    size: asset.fileSize ?? null,
    kind: "image",
  }
...
export async function captureChatVideo(): Promise<PickedChatAttachment | null> {
  const permission = await ImagePicker.requestCameraPermissionsAsync()
  if (!permission.granted) return null
  ...
  return {
    uri: asset.uri,
    name: safeFileName(asset.fileName || capturedName("video", mimeType)),
    mimeType,
    size: asset.fileSize ?? null,
    kind: "video",
  }
